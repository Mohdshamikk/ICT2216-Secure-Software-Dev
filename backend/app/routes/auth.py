...
def register():
    ...
    log_event('USER_REGISTERED', 'SUCCESS', _ip(), user_id=user.id, user_agent=_ua())
    return jsonify(_generic), 201
...
def verify_email():
    ...
    log_event('EMAIL_VERIFIED', 'SUCCESS', _ip(), user_id=user.id, user_agent=_ua())
...
def logout():
    ...
    db.session.commit()
    log_event('LOGOUT', 'SUCCESS', _ip(), user_id=g.current_user.id, user_agent=_ua())
...
def mfa_setup():
    ...
    log_event('MFA_SETUP_INITIATED', 'SUCCESS', _ip(), user_id=user.id, user_agent=_ua())
